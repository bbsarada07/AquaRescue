...
>>>>>>> Stashed changes
  ...
  const panelContent: Record<PanelId, React.ReactNode> = {
    'active-target': activeTargetContent,
    'kalman-gps': kalmanGpsContent,
    'raw-gps': (
      <div className="space-y-2 text-xs">
        <div className="grid grid-cols-2 gap-y-1.5 font-mono">
          <span className="text-gray-500 text-[10px] uppercase">Raw Lat</span>
          <span className="font-bold text-[#06B6D4]">{(rawLocation?.lat ?? targetLat).toFixed(6)}</span>
          <span className="text-gray-500 text-[10px] uppercase">Raw Lng</span>
          <span className="font-bold text-[#06B6D4]">{(rawLocation?.lng ?? targetLng).toFixed(6)}</span>
          <span className="text-gray-500 text-[10px] uppercase">Mode</span>
          <span className="text-[#F59E0B] font-bold">MULTIPATH NOISY</span>
        </div>
      </div>
    ),
    'drone-camera': <DroneCameraFeed {...droneCameraProps} />,
    'drone-sensors': (
      <div className="space-y-2.5 text-xs">
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-[#111827] border border-[#1A2840] rounded p-2"><div className="text-gray-500 text-[9px] uppercase">Camera</div><div className="font-bold text-[#10B981]">{cameraMode} ACTIVE</div></div>
          <div className="bg-[#111827] border border-[#1A2840] rounded p-2"><div className="text-gray-500 text-[9px] uppercase">Target</div><div className="font-bold text-[#10B981]">PERSON LOCKED</div></div>
          <div className="bg-[#111827] border border-[#1A2840] rounded p-2"><div className="text-gray-500 text-[9px] uppercase">Distance</div><div className="font-bold">{Math.round(droneDist)} m</div></div>
          <div className="bg-[#111827] border border-[#1A2840] rounded p-2"><div className="text-gray-500 text-[9px] uppercase">ETA</div><div className="font-bold">{formatEtaClock(droneEta)}</div></div>
        </div>
        <div className="border-t border-[#1A2840] pt-2">
          <div className="text-[#67e8f9] font-bold mb-1 text-[10px] uppercase">Life Jacket Payload</div>
          <div className="grid grid-cols-2 gap-y-1">
            <span className="text-gray-500 text-[10px] uppercase">Status</span><span className="text-[#10B981] font-bold">READY</span>
            <span className="text-gray-500 text-[10px] uppercase">Target Lock</span><span className="text-[#10B981] font-bold">✓</span>
            <span className="text-gray-500 text-[10px] uppercase">Drop Zone</span><span className="text-[#10B981] font-bold">LOCKED</span>
          </div>
          <button
            onClick={onManualPayloadDrop}
            className="mt-2 w-full py-1.5 rounded border border-[#06b6d4]/50 bg-[#06b6d4]/10 text-[#67e8f9] font-bold text-[10px] uppercase hover:bg-[#06b6d4]/20 transition-all"
          >
    ...
    'rescue-team': rescueTeamContent,
    'audio-analysis': audioAnalysisContent,
    hydrodynamics: hydrodynamicsContent,
    ...
    'mission-controls': missionControlsContent,
  };
